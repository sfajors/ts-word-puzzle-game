...
const boards = [
  ...
  },
  {
    ...
    words: [
      ...
      { 
        word: 'EONS', 
        hint: 'It’s a very, very long period of time, often used to describe Earth\'s history.', 
        additionalHint: 'This word means a time so long it feels like forever!', 
        attempts: 0,
        found: false,
        failed: false,
        positions: []
...
// Removed wordsToFindElement since the word bank is removed

// Define cell properties
let cellSize = 50; // Will be recalculated based on canvas size
let radius = cellSize / 2 - 5; // Radius for circles with padding
const cols = gridLetters[0].length;
const rows = gridLetters.length;
let boardOffsetX = 0; // Will be recalculated based on canvas size
let boardOffsetY = 0; // Will be recalculated based on canvas size
...
function resizeCanvas() {
  ...
  // Set a square canvas or based on grid aspect ratio
  ...
  // Recalculate cellSize and radius based on new canvas size
  cellSize = canvas.width / cols;
  radius = cellSize / 2 - 5;

  // Recalculate board offsets to center the grid
  boardOffsetX = (canvas.width - (cellSize * cols)) / 2;
  boardOffsetY = (canvas.height - (cellSize * rows)) / 2;
...
function drawBoard() {
  ...
  ctx.font = `${cellSize / 2}px Arial`; // Adjust font size based on cell size
...
function showCurrentHint() {
  ...
  if (!wordObj) {
    hintElement.textContent = 'Congratulations! You found all the words!';
    return;
  }
  let hintText = wordObj.hint;
  if (wordObj.attempts >= 3 && wordObj.additionalHint) {
    hintText += `\nAdditional Hint: ${wordObj.additionalHint}`;
  }
...
function checkSelectedWord() {
  ...
  if (selectedWordStr === wordObj.word.toUpperCase()) {
    ...
    // Removed updateWordList since word bank is removed
  }
...
// Highlight the failed word's letters on the board
...
function showCongratulations() {
  ...
  if (finalQuestionDiv) {
    ...
    document.getElementById('game-container').appendChild(congratsDiv);
  }
}
...
// Initialize the game
function initGame() {
  // Removed createWordList since word bank is removed
  showCurrentHint();
  drawBoard();
}
